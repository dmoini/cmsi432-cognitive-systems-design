# myTeam.py (Donovan Moini, Serena Zafiris, Lucille Njoo)
# ---------------
...
class QLearningAgent(CaptureAgent):
    ...
    def getNumberOfAdjacentWalls(self, state, queryPos):
        x, y = queryPos
        numberOfWalls = 0
...
class OffensiveAgent(QLearningAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        # if self.isPacman(successor): #or self.isScaredGhost(gameState, self.index):  
        ...
        """
        adjacentWalls = self.getNumberOfAdjacentWalls(successor, successorPos)
        # We only care about nearby walls when there is nonzero risk of enemy killing us
        if features['DistanceToNearestEnemy'] != 0.0:
            features["NumberOfAdjacentWalls"] = float(adjacentWalls - 1) / 4.0
        else:
            features["NumberOfAdjacentWalls"] = 0.0
