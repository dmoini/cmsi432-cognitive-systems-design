...
class QLearningAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        pprint(self.explorations)
        self.explorationConstant = 2.0 if TRAINING else 0.0
        ...
        # self.safePositions = [(15, 1), (15, 2), (15, 4), (15, 5), (15, 7), (15, 8), (15, 11), (15, 12), (15, 13), (15, 14)]
    ...
    def chooseAction(self, gameState):
        ...
        if self.pickedUpFood(s, s_prime):
            self.collectedFood += 1
        if self.depositedFood(s, s_prime):
    ...
    def getOptimisticQValue(self, gameState, action):
        ...
        bias = self.explorationConstant / (self.explorations[(position, action)] + 1)
    ...
    def generateDefaultWeights(self):
        ...
        for feature in FEATURES["offense" if self.isOffensiveAgent() else "defense"]:
            weights[feature] = ((random.random() * 2.0) - 1.0) / 10.0
        return weights
    ...
    def getDistanceToNearestEnemyFromPosition(self, currentState, queryPos):
        ...
        nearestEnemyPos = currentState.getAgentPosition(nearestEnemyIndex) if (nearestEnemyIndex is not None) else None
        print(" -- nearestEnemyPos is " + str(nearestEnemyPos))
        distance = self.distancer.getDistance(queryPos, nearestEnemyPos) if (nearestEnemyPos is not None) else self.maxMazeDistance
    ...
    # def getDistanceToNearestFoodFromPosition(self, currentState, potentialPos):
    #     foodList = self.getFood(currentState).asList()
    #     if len(foodList) > 0:  # This should always be True, but better safe than sorry
    #         minDistance = min([self.getMazeDistance(potentialPos, foodPos) for foodPos in foodList])
    #     return minDistance
    ...
    def isScaredGhost(self, gameState):
        return gameState.getAgentState(self.index).scaredTimer > 0.0
...
class OffensiveAgent(QLearningAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        """
        if self.isPacman(gameState) or self.isScaredGhost(gameState):  
            ...
            normalizedDistanceToNearestEnemy = float(distanceToNearestEnemy + 1.0) / self.maxMazeDistance # add a +1 to the numerator so that when we're going to be right on top of an enemy, we have a nonzero value
            # if we couldn"t see any enemies in the first place:
            if normalizedDistanceToNearestEnemy >= 1.0 or (distanceToNearestEnemy != self.maxMazeDistance and distanceToNearestEnemy >= 10): 
                ...
                print(" -- normalizedDistanceToNearestEnemy: " + str(normalizedDistanceToNearestEnemy))
                features["DistanceToNearestEnemy"] = 0.0
            # otherwise, we saw an enemy
            else:    
                print("enemy is close so we care, setting feature to normalized value: " + str(normalizedDistanceToNearestEnemy))
                features["DistanceToNearestEnemy"] = normalizedDistanceToNearestEnemy 
                # NJOOB IS FUCKING AROUND
                # features["DistanceToNearestEnemy"] = 1.0 - (float(distanceToNearestEnemy) / self.maxMazeDistance)

        """
        Distance to the nearest Food (pellet)
        """
        if self.justDied == 0:
            # We know that food is one move away if the current gameState has edible food in that position we"re going to be in, 
            # and then the following state does NOT have food at that position
            foodIsCurrentlyOneMoveAway = self.hasEdibleFoodAtPosition(gameState, successorPos[0], successorPos[1]) \
                and not self.hasEdibleFoodAtPosition(successor, successorPos[0], successorPos[1])
            distanceToNearestFood = 0.0 if foodIsCurrentlyOneMoveAway else self.getDistanceToNearestFood(successor) 
            normalizedDistanceToNearestFood = -float(distanceToNearestFood + 1.0) / self.maxMazeDistance 
            features["DistanceToNearestFood"] = normalizedDistanceToNearestFood
            # NJOOB IS FUCKING AROUND
            # features["DistanceToNearestFood"] = -( 1.0 - ( float(distanceToNearestFood) / float(self.maxMazeDistance) ) )
        else:
        ...
        #     features["Safety"] = 0.0
        # else:
        #     features["Safety"] = 1.0 - (float(self.collectedFood) * float(self.getDistanceFromNearestSafeState(successor)) / 500.0)
