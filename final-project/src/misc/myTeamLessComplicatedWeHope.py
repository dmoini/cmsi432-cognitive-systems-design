...
class QLearningAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        print("CURRENT POS: " + str(gameState.getAgentPosition(self.index)) + ", IS PACMAN: " + str(self.isPacman(gameState)))
    ...
    def readWeightsFromFile(self):
        ...
        f.close()
        return self.byteify(weightsDict)

    def writeWeightsToFile(self):
        updatedWeights = self.readWeightsFromFile() if path.exists(
            WEIGHTS_FILE) else deepcopy(EMPTY_WEIGHTS)
        updatedWeights["offense" if self.isOffensiveAgent() else "defense"] = self.weights
    ...
    def initializeWeights(self):
        if path.exists(WEIGHTS_FILE):
            ...
            weightsDict = self.readWeightsFromFile().get(
...
class OffensiveAgent(QLearningAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        """
        # TODO LUCILLE put all of this in a check to see if we're Pacman OR a scared ghost; otherwise we don't care
        # distanceToNearestEnemy = self.getDistanceToNearestEnemy(successor) 
        distanceToNearestEnemy = self.getDistanceToNearestEnemyFromPosition(gameState, successorPos) 
        normalizedDistanceToNearestEnemy = (distanceToNearestEnemy + 1.0) / self.maxMazeDistance
        # if we couldn't see any enemies in the first place:
        if normalizedDistanceToNearestEnemy >= 1.0 or distanceToNearestEnemy >= 10: 
        ...
        """
        if self.justDied == 0:
        # if self.justDied == 0 or : # TODO LUCILLE perhaps check later for whether we we are too close to enemy
            successorPos = self.getNewPositionFromTakingLegalMove(gameState, action)
            distanceToNearestFood = self.getDistanceToNearestFood(successor) # returns maxMazeDistance if no enemies are visible
            ...
            foodIsOneMoveAway = self.hasEdibleFoodAtPosition(gameState, successorPos[0], successorPos[1]) \
                and not self.hasEdibleFoodAtPosition(successor, successorPos[0], successorPos[1])
            if foodIsOneMoveAway:
                distanceToNearestFood = 1.0
                # features['DistanceToNearestFood'] = 1.0
            features['DistanceToNearestFood'] = -(distanceToNearestFood + 1.0) / self.maxMazeDistance 
        else:
        ...
        """
        safety = -(self.collectedFood * self.getDistanceFromInitialState(successor)) / 200.0 #(self.totalFood * self.maxMazeDistance)
        # if self.depositedFood(gameState, successor): 
        #     features['Safety'] = 0.0
        # else:
